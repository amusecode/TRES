...
def rdc(file_name_root, print_style, print_full, print_init, line_number, inner_primary_star_type, inner_secondary_star_type, 
inner_primary_star_type_string, inner_secondary_star_type_string,  
inner_bin_type, inner_bin_type_string, triple_type, triple_type_string):
    ...
    if print_init:
        for i, triple in enumerate(triple.history):
            if i == line_number:
                ...
                print(' -M ', triple[0].child1.mass.value_in(units.MSun), ' -m ', triple[0].child2.mass.value_in(units.MSun), ' -l ', end = '')
                print(' -A ', triple[0].semimajor_axis.value_in(units.RSun), end = '')
                print(' -E ', triple[0].eccentricity, end = '')
                print(' -G ', triple[0].argument_of_pericenter, end = '\t')
...
#                    triple_type_snapshot = tr_type['semisecular_regime']
        ...
        if print_style == 0:
            ...
            snapshot_string = snapshot_string + print_to_string(i, triple[0].number, triple[0].time, triple[0].error_flag_secular, triple[0].CPU_time)
            snapshot_string = snapshot_string + print_to_string('bin:', triple[0].bin_type, triple[0].semimajor_axis, triple[0].eccentricity, triple[0].argument_of_pericenter, triple[0].longitude_of_ascending_node)
            snapshot_string = snapshot_string + print_to_string('st:', triple[0].child1.is_donor, triple[0].child1.stellar_type, triple[0].child1.mass, triple[0].child1.spin_angular_frequency, triple[0].child1.radius, triple[0].child1.core_mass)
            snapshot_string = snapshot_string + print_to_string('|', triple[0].child2.is_donor, triple[0].child2.stellar_type, triple[0].child2.mass, triple[0].child2.spin_angular_frequency, triple[0].child2.radius, triple[0].child2.core_mass)
            ...
            if print_style == 3:
                snapshot_string = snapshot_string + print_to_string_csv(triple[0].number, triple[0].time.value_in(units.Myr), int(triple[0].error_flag_secular), triple[0].CPU_time, end = ',')
                ...
                snapshot_string = snapshot_string + print_to_string_csv(int(triple[0].child2.is_donor), triple[0].child2.stellar_type.value_in(units.stellar_type), triple[0].child2.mass.value_in(units.MSun), triple[0].child2.spin_angular_frequency.value_in(1./units.Myr), triple[0].child2.radius.value_in(units.RSun), triple[0].child2.core_mass.value_in(units.MSun))

            else:
                snapshot_string = snapshot_string + print_to_string(triple[0].number, triple[0].time.value_in(units.Myr), int(triple[0].error_flag_secular), triple[0].CPU_time, end = '\t')
                ...
                snapshot_string = snapshot_string + print_to_string(int(triple[0].child2.is_donor), triple[0].child2.stellar_type.value_in(units.stellar_type), triple[0].child2.mass.value_in(units.MSun), triple[0].child2.spin_angular_frequency.value_in(1./units.Myr), triple[0].child2.radius.value_in(units.RSun), triple[0].child2.core_mass.value_in(units.MSun))
#            snapshot_string = snapshot_string + '\n'

        if (inner_primary_star_type == star_type['all'] or triple[0].child1.stellar_type.value_in(units.stellar_type) in np.array(inner_primary_star_type)) and \
        (inner_secondary_star_type == star_type['all'] or triple[0].child2.stellar_type.value_in(units.stellar_type) in np.array(inner_secondary_star_type)) and \
